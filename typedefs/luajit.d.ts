/**
 * Ambient global type definitions for the base Lua standard library methods available in the X-Ray
 * engine build that TypeScriptToLua's own typings omit.
 *
 * These are ambient globals, so reference this file from a consumer instead of importing it, e.g. add
 * `"xray16/typedefs/luajit"` to the `types` array of your tsconfig or use
