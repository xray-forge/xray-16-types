...
/**
 * Runtime stand-in for the bare `xray16` engine module under Jest/Node.
 *
 * `createJestConfig` maps `^xray16$` here, and `setupXrayRuntime` reuses these export values when it
 * registers an explicit Jest module mock. Tests can spy on exported singleton objects such as `level` and
 * observe the same objects from code that imports `xray16`. The real `xray16` module only exists inside the
 * game process.
 */
...
export {
  ...
  MockCUIDialogWnd as CUIDialogWnd,
  MockCUIEditBox as CUIEditBox,
  MockCUIGameCustom as CUIGameCustom,
  MockCUILines as CUILines,
  ...
  MockCUIMessageBox as CUIMessageBox,
