/**
 * Friendly type aliases over the raw `xray16` engine binding names, plus engine `@virtual` enums.
 *
 * The type aliases are pure types (no runtime). The enums are `@virtual`, so the `inline` plugin folds their
 * members into literals in the emitted Lua (no runtime table), while jest/node get the real enum objects from
 * the compiled `alias.js`. Re-exportable by consumers as `xray16/alias`.
 */
import type {
...
export type Artefact = CArtefact;
